...
class PostList(generic.ListView):
    model = Post
    ordering = '-created'
    template_name = 'news_board/post_list.html'
    context_object_name = 'post_list'


class PostDetail(generic.DetailView):
    model = Post
    template_name = 'news_board/post_detail.html'


class PostCreate(LoginRequiredMixin, generic.CreateView):
    model = Post
    template_name = 'news_board/post_edit.html'
...
class PostUpdate(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_template_names(self):
        ...
        if post.user == self.request.user:
            self.template_name = 'news_board/post_edit.html'
...
class ResponseList(LoginRequiredMixin, generic.ListView):
    model = Response
    template_name = 'news_board/response_list.html'
...
class ResponseDetail(LoginRequiredMixin, generic.DetailView):
    ...
    def get_template_names(self):
        ...
        if response.post.user == self.request.user:
            self.template_name = 'news_board/response_detail.html'
            return self.template_name
        else:
            raise PermissionDenied
...
class ResponseCreate(LoginRequiredMixin, generic.CreateView):
    model = Response
    template_name = 'news_board/response_create.html'
    ...
    def form_valid(self, form):
        ...
        send_mail(
            ...
            from_email='evgen23strelnikov@yandex.ru',
...
class SuccessView(LoginRequiredMixin, generic.TemplateView):
    template_name = 'news_ board/success.html'
...
def accept_response(request, pk):
    ...
    send_mail(
        ...
        from_email='evgen23strelnikovk@yandex.ru',
